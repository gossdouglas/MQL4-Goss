   ...
   for (int i = 0; i <= Bars; i++)
      {
         ...
         DeltaCollapsedPosHistoryBuffer[i] = iCustom(Symbol(),1, duto_chart_indicators, 1, i);
...
{  
   ...
   if (CombinedHistory[1][36] > 0 && CombinedHistory[2][36] < 0 && CombinedHistory[1][35] == 1)
   {
      Print("Macd M1 one candle back >0 , Macd M1 two candles back <0, Collapsed D one candle back positive, buy: " + NormalizeDouble(CombinedHistory[1][36], 6) + ", " + NormalizeDouble(CombinedHistory[2][36], 6) + ", " + CombinedHistory[1][35]);
...

{
   ...
   // This is where you should insert your Entry Signal for BUY orders
   // Include a condition to open a buy order, the condition will have to set SignalEntry=SIGNAL_ENTRY_BUY
   if (CombinedHistory[1][36] < 0 && CombinedHistory[2][36] > 0)
   {
      Print("Macd M1 one candle back < 0 , Macd M1 two candles back > 0, exit buy: " + NormalizeDouble(CombinedHistory[1][36], 6) + ", " + NormalizeDouble(CombinedHistory[2][36], 6));
