...
{  
   ...
   if (CombinedHistory[1][16] > 0 && CombinedHistory[2][16] < 0 && CombinedHistory[1][15] == 1)
   {
      Print(
         "Macd M15 idx 1 > 0: " + CombinedHistory[1][16] + ", "
         "Macd M15 idx 2 < 0:"  + CombinedHistory[2][16] + ", "
         "Collapsed D idx 1 positive: " + CombinedHistory[1][15] +
         "buy");
   ...
   if (
      CombinedHistory[1][16] < CombinedHistory[2][16] 
      && CombinedHistory[1][16] > 0 
      && CombinedHistory[2][16] > 0

      && CombinedHistory[1][17] < 0
      && CombinedHistory[1][18] < 0
      && CombinedHistory[1][19] < 0
...

{
   ...
   // Include a condition to open a buy order, the condition will have to set SignalEntry=SIGNAL_ENTRY_BUY
   if (CombinedHistory[1][15] == -1)
   {
      Print(
         //"Macd M15 idx 1 > 0: " + CombinedHistory[1][16] + ", "
         //"Macd M15 idx 2 < 0:"  + CombinedHistory[2][16] + ", "
         "Collapsed D idx 1 negative: " + CombinedHistory[1][15] +
         "exit buy");
