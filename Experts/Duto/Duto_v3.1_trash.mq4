...
void ExecuteEntry()
{
   ...
   if (SignalEntry == SIGNAL_ENTRY_SELL)
   {
      ...
      // Submit the order
      //SendOrder(Operation, Symbol(), OpenPrice, StopLossPrice, TakeProfitPrice);
      SendOrder(Operation, Symbol(), CombinedHistory[1][32], StopLossPrice, TakeProfitPrice);
   }
}
...
void SendOrder(int Command, string Instrument, double OrderPrice, double SLPrice, double TPPrice, datetime Expiration = 0)
{
   ...
   for (int i = 1; i <= OrderOpRetry; i++)
   {
      ...
      if (SLPrice > 0)
         ...
         MathCeil(MathAbs(OrderPrice - SLPrice) / Point);
      ...
      int res = OrderSend(Instrument, Command, LotSize, OrderPrice, Slippage, NormalizeDouble(SLPrice, Digits), NormalizeDouble(TPPrice, Digits), OrderNote, MagicNumber, Expiration, OpenColor);
      ...
      if (res)
      {
         ...
         Print("TRADE - OPEN SUCCESS - Order ", res, " submitted: Command ", Command, " Volume ", LotSize, " Order Price ", OrderPrice, " Stop ", SLPrice, " Take ", TPPrice, " Expiration ", Expiration);
         break;
      }
      ...
      else
      {
         ...
         Print("TRADE - OPEN FAILED - Order ", res, " submitted: Command ", Command, " Volume ", LotSize, " Order Price ", OrderPrice, " Stop ", SLPrice, " Take ", TPPrice, " Expiration ", Expiration);
         int Error = GetLastError();
         string ErrorText = GetLastErrorText(Error);
