...
extern int  MainFilterFast    =    15;
extern int  MainFilterSlow    =   25;
extern int  SmallFilterFast   =    9;
extern int  SmallFilterSlow   =   12;
extern bool FirstSignalOnly   = true;
extern bool AlertMode         = true;
extern int  AlertsBar         =0;
...
int init() 
{
   ...
   SetIndexBuffer(0,signal_UP); SetIndexStyle(0,DRAW_ARROW,EMPTY,5); SetIndexArrow(0,233);
   //SetIndexBuffer(1,signal_DN); SetIndexStyle(1,DRAW_ARROW); SetIndexArrow(1,234);
   SetIndexBuffer(1,signal_DN); SetIndexStyle(1,DRAW_ARROW,EMPTY,5); SetIndexArrow(1,234);
...
int start() 
{
   ...
   limit = MathMax(limit,TimeFrame/Period());   
      ...
      Print("signal_UP[" + shift + "]: " + signal_UP[shift]);
   ...
   if(signal_UP[AlertsBar] != EMPTY_VALUE && signal_UP[AlertsBar] != 0.0 && DnTrend && Bars>barnumber)
   {
      barnumber = Bars;
      DnTrend   = FALSE;
   ...
   if (!DnTrend && (signal_UP[AlertsBar] == EMPTY_VALUE || signal_UP[AlertsBar] == 0.0)) DnTrend = TRUE;
   
   if (signal_DN[AlertsBar] != EMPTY_VALUE && signal_DN[AlertsBar] != 0.0 && UpTrend && Bars > barnumber)
   {
      barnumber=Bars;
      UpTrend = FALSE;
   ...
   if ((!UpTrend) && (signal_DN[AlertsBar] == EMPTY_VALUE || signal_DN[AlertsBar] == 0.0)) UpTrend = TRUE;
